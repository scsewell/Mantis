...
#define LOG_TAG MANTIS_TEXT("GraphicsPipline")

namespace Mantis
{
    const eastl::vector<VkDynamicState> DYNAMIC_STATES =
    {
    ...
    PipelineGraphics::PipelineGraphics(
        ...
        m_shader(eastl::make_unique<Shader>()),
        ...
        m_pipelineBindPoint(VK_PIPELINE_BIND_POINT_GRAPHICS)
    {
        ...
        eastl::sort(m_vertexInputs.begin(), m_vertexInputs.end());
        CreateShaderProgram();
        ...
        switch (m_mode)
        {
            ...
            case Mode::Mrt:
                ...
                break;
            default:
                Logger::ErrorT(LOG_TAG, "Unknown pipline mode!");
                break;
        }

#if defined(MANTIS_DEBUG)
        Logger::DebugTF(LOG_TAG, "Pipeline graphics \"%ls\" created in %.3fms", m_shaderStages.back(), (Timer::Now() - startTime).AsMilliseconds<float>());
#endif
    }
    ...
    const ImageDepth* PipelineGraphics::GetDepthStencil(const eastl::optional<uint32_t>& stage) const
    {
        return Graphics::Get()->GetRenderStage(stage ? *stage : m_stage.first)->GetDepthStencil();
    }

    const Image2d* PipelineGraphics::GetImage(const uint32_t & index, const eastl::optional<uint32_t> & stage) const
    {
        return Graphics::Get()->GetRenderStage(stage ? *stage : m_stage.first)->GetFramebuffers()->GetAttachment(index);
    }

    RectInt PipelineGraphics::GetRenderArea(const eastl::optional<uint32_t> & stage) const
    {
