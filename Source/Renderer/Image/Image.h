...
namespace Mantis
{
    ...
    public:
         ...
         */
        eastl::unique_ptr<uint8_t[]> GetPixels(VkExtent3D& extent, const uint32_t& mipLevel = 0, const uint32_t& arrayLayer = 0) const;
        ...
        static eastl::unique_ptr<uint8_t[]> LoadPixels(const std::string& filename, Vector2Int& extent, uint32_t& components, VkFormat& format);

        static void WritePixels(const String& filename, const uint8_t* pixels, const Vector2Int& extent, const int32_t& components = 4);
